...
// lang can be one of (ua, ru or en)
let [, , trackNumber, lang, telegramToken, telegramChatId] = process.argv;
lang = lang || 'en';
...
(async () => {
    ...
    const salt = /salt = \'(.*)\'/.exec(pageResponse.data)[1];
    const url = `${BASE_URL}/get.php?what=tracking&number=${trackNumber}&ext_track=&chk=${check(salt + trackNumber + salt)}`;

    await printResult((await xml2js((await axios.post(url)).data)).return.result_table[0].items);
})();
